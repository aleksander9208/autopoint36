...
<div class="col-md-12 col-sm-12">
	<?if (!count($arResult['ORDERS']))
	{
		if ($_REQUEST["filter_history"] == 'Y')
		{
			...
			{
				?>
				<h3 class="order__title"><?= Loc::getMessage('SPOL_TPL_EMPTY_CANCELED_ORDER')?></h3>
				<?
			}
			else
			{
				?>
				<h3 class="order__title"><?= Loc::getMessage('SPOL_TPL_EMPTY_HISTORY_ORDER_LIST')?></h3>
				<?
			}
		}
		else
		{
			?>
			<h3 class="order__title"><?= Loc::getMessage('SPOL_TPL_EMPTY_ORDER_LIST')?></h3>
			<?
		}
	}
	?>
	...
	<div class="col-md-12 col-sm-12">
		...
		{
			?>
			<a class="sale-order-history-link def-link" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y", $clearFromLink, false)?>">
				<?echo Loc::getMessage("SPOL_TPL_VIEW_ORDERS_HISTORY")?>
			</a>
			<?
		}
		if ($_REQUEST["filter_history"] == 'Y' && count($arResult['ORDERS'])>0)
		{
			?>
			<a class="sale-order-history-link def-link" href="<?=$APPLICATION->GetCurPageParam("", $clearFromLink, false)?>">
			...
			{
				?>
				<a class="sale-order-history-link def-link" href="<?=$APPLICATION->GetCurPageParam("filter_history=Y&show_canceled=Y", $clearFromLink, false)?>">
	...
	if (!count($arResult['ORDERS']))
	{
		?>
		<div class="col-md-12 col-sm-12">
			<a href="<?=htmlspecialcharsbx($arParams['PATH_TO_CATALOG'])?>" class="sale-order-history-link def-link">
	...
	if ($_REQUEST["filter_history"] !== 'Y')
	{
		...
		foreach ($arResult['ORDERS'] as $key => $order)
		{
			if ($orderHeaderStatus !== $order['ORDER']['STATUS_ID'] && $arResult['SORT_TYPE'] == 'STATUS')
			{
				...
				?>
				<div class="col-md-12 sale-order-title">
					...
					<div class="col-md-12 sale-order-list-inner-container">
						...
						<div class="sale-order-list-inner-row">
							...
							<div class="col-md-<?=($order['ORDER']['CAN_CANCEL'] !== 'N') ? 8 : 10?>  col-sm-12 sale-order-list-about-container">
								<a class="sale-order-list-about-link def-link" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_DETAIL"])?>"><?=Loc::getMessage('SPOL_TPL_MORE_ON_ORDER')?></a>
							</div>
					...
					<div class="col-md-12 sale-order-list-inner-accomplished">
						<div class="row sale-order-list-inner-row">
							<div class="col-md-3 col-sm-12 sale-order-list-about-accomplished">
								<a class="sale-order-list-about-link def-link" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_DETAIL"])?>">
									<?=Loc::getMessage('SPOL_TPL_MORE_ON_ORDER')?>
								</a>
							</div>
							<div class="col-md-3 col-md-offset-6 col-sm-12 sale-order-list-repeat-accomplished">
                                <span class="icon orderHistory__icon">
                                    <i class="icon-shoping-cart-outline"></i>
                                </span>
								<a class="sale-order-link-accomplished def-link" href="<?=htmlspecialcharsbx($order["ORDER"]["URL_TO_COPY"])?>">
